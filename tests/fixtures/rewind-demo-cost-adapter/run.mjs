...
//
// Usage: node tests/fixtures/rewind-demo-cost-adapter/run.mjs

import path from 'node:path';
import { locate, run } from '../_harness.mjs';

const PY = process.platform === 'win32' ? 'python' : 'python3';
const { fixtureDir } = locate(import.meta.url);

run(PY, [path.join(fixtureDir, 'check_cost_adapter.py'), fixtureDir]);
