...
# its own action_type — all through the injected capture channel. The whole point
...
import rewind_client  # noqa: E402  # pure-stdlib hook, injected on PYTHONPATH by the supervisor
...
# 2. register the schema into the run, and emit an event of its action type
if not rewind_client.setup():
    print("capture channel not announced", file=sys.stderr)
    sys.exit(1)
rewind_client.kfx_schema(KFX_ACTION_TYPE, "KfxEvent", bfbs, tier="trusted")
...
rewind_client.kfx_event(KFX_ACTION_TYPE, bytes(builder.Output()))
