...
The generic Profile runtime binds this module to exact Suite/member roots.
"""
...
def _action(domain, operation: str, runtime_dir: str, values: Mapping[str, Any]):
    initiative_bundle = domain.initiative_bundle
    ...
    if operation == "create-initiative":
        ...
        receipt = initiative_bundle.write_initiative_bundle(
            runtime_dir,
        ...
        receipt = initiative_bundle.import_initiative_bundle_file(
            runtime_dir,
...
def _invoke(
    ...
    if operation == "initiative":
        _only(values, {"initiativeId"}, operation)
        state = domain.work_control.query_state(
            runtime_dir, initiative_id=str(values.get("initiativeId") or "")
        )
        return {
            "initiative": state["initiative"]["payload"]["record"],
            "assignments": [row["payload"]["record"] for row in state["assignments"]],
        }
    ...
    if operation == "authority-status":
        _only(values, set(), operation)
        return domain.work_control.authority_status(runtime_dir)
