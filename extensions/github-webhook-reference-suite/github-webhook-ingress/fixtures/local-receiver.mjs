...
export async function startLocalReceiver(host, maximum = 262_144) {
  const server = createServer(async (request, response) => {
    try {
      ...
      response.end(
        JSON.stringify({ accepted: false, code: 'FIXTURE_RECEIVER_FAILED' }),
      );
    }
  });
  ...
  return {
    url: `http://127.0.0.1:${address.port}/github/events`,
