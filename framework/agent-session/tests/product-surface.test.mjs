...
test('one live native Agent session serially observes independent Work items', () => {
  const { clients, input } = fixture();
  const assistantBinding = { kind: 'workspace-assistant', workRef: null };
  const firstIdentity = { launcherPid: 42, launchedAt: 4000 };
  const first = clients.cli.planNativeStart({
    ...input,
    workspaceId: 'workspace-1',
    workConsoleId: 'assistant:workspace-1:native:first',
    binding: assistantBinding,
  });
  clients.cli.startNative(first, firstIdentity);
  const second = clients.cli.planNativeStart({
    ...input,
    workspaceId: 'workspace-1',
    workConsoleId: 'assistant:workspace-1:native:second',
    sessionAttemptId: 'attempt:assignment-42:second',
    provider: 'opencode',
    providerVersion: '1.18.3',
    bootstrap: {
  ...
  const firstWorkRef = input.binding.workRef;
  assert.equal(
    clients.cli.bindNativeWork(
      clients.cli.planNativeBindWork(first, firstWorkRef),
    ).status,
    'bound',
  );
  clients.cli.projectNativeWork(
    ...
    nativeProjection({
      ...
      assignmentId: 'assignment-42',
      title: 'First Work',
      objective: 'Prove stale projections do not cross Work boundaries',
      acceptanceChecks: ['Switch Work without restarting the Agent'],
      phase: 'executing',
      queryProofRoot: `sha256:${'b'.repeat(64)}`,
      nextActions: ['finish-first-work'],
      ...
      remainingObligation: 'finish-first-work',
      nextAction: 'finish-first-work',
    }),
  );
  assert.notEqual(clients.gui.show(first).nativeObserver.workProjection, null);
  const secondWorkRef = {
    ...firstWorkRef,
    entityId: 'assignment-43',
    entityRoot: `sha256:${'a'.repeat(64)}`,
    systemTimeCut: '2026-08-09T09:00:00Z',
  };
  assert.equal(
    clients.cli.bindNativeWork(
      clients.cli.planNativeBindWork(first, secondWorkRef),
    ).status,
    'bound',
  );

  const firstStatus = clients.gui.show(first);
  assert.equal(firstStatus.lifecycleState, 'running');
  assert.equal(firstStatus.binding.workRef.entityId, 'assignment-43');
  assert.equal(firstStatus.nativeObserver.workProjection, null);
  assert.equal(clients.gui.list().activeWorkLeases.length, 1);
  assert.equal(
    clients.gui.list().activeWorkLeases[0].workRef.entityId,
    'assignment-43',
  );

  assert.equal(
    clients.cli.bindNativeWork(
      clients.cli.planNativeBindWork(second, firstWorkRef),
    ).status,
    'bound',
  );
  assert.throws(
    () => clients.cli.planNativeBindWork(second, secondWorkRef),
    (error) => error.code === 'native_work_already_active',
  );
  const leasesAfterDeniedSwitch = clients.gui.list().activeWorkLeases;
  assert.equal(leasesAfterDeniedSwitch.length, 2);
  assert.equal(
    leasesAfterDeniedSwitch.find(
      (lease) => lease.workRef.entityId === 'assignment-42',
    ).sessionAttemptId,
    second.sessionAttemptId,
  );
  assert.equal(
    leasesAfterDeniedSwitch.find(
      (lease) => lease.workRef.entityId === 'assignment-43',
    ).sessionAttemptId,
    first.sessionAttemptId,
  );
});
