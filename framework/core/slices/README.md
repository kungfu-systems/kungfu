...
| `view-encapsulation/` | All C++ FlatBuffers/reflection access lives behind one runtime-independent chokepoint (`kungfu::view`, KF-ADR-019f86da-4f90-7a66-b427-f4bcd638d8bc): the open-layer `.bfbs` projection round-trips a frame to SQLite and back through the view API alone (built without linking the runtime), and no raw `flatbuffers::`/`reflection::` symbol appears outside the module — so the `.bfbs` dangling-view bug is structurally unrepresentable. |
