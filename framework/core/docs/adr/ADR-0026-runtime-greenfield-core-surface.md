...
Narrow the public runtime binding surface to neutral runtime primitives:
...
Add `scripts/check-runtime-greenfield.mjs` to prevent these specific patterns
from returning.
...
- **Delete every historical schema definition in one change.** Rejected for this
  ...
  stop exposing and extending old trading/profile concepts as core runtime
  APIs.
