...
// k-way merge contract: across journals, frames come out in gen_time order
// regardless of which journal produced them.
void test_reader_merges_journals_in_gen_time_order() {
  temp_tree tree;
  auto loc = make_location(tree.root());
  // Interleaved on purpose: no single journal is already globally ordered.
  seed_journal_at(loc, 1, {10, 40, 70});
  seed_journal_at(loc, 2, {20, 50, 80});
  seed_journal_at(loc, 3, {30, 60, 90});

  reader target(reader_policy::peer(), true, make_bus());
  target.join(loc, 1, 0, TEST_PAGE_SIZE_MB);
  target.join(loc, 2, 0, TEST_PAGE_SIZE_MB);
  target.join(loc, 3, 0, TEST_PAGE_SIZE_MB);

  const auto observed = drain(target, 9);
  require(observed.size() == 9, "reader did not surface every joined frame");
  const std::vector<int64_t> expected{10, 20, 30, 40, 50, 60, 70, 80, 90};
  for (size_t i = 0; i < expected.size(); ++i) {
    require(observed[i].first == expected[i], "reader merged frames out of gen_time order at index " +
                                                  std::to_string(i) + ": expected " + std::to_string(expected[i]) +
                                                  ", got " + std::to_string(observed[i].first));
  }
}
...
int main() {
  const std::pair<const char *, void (*)()> tests[] = {
      ...
      {"reader merges journals in gen_time order", test_reader_merges_journals_in_gen_time_order},
