...
public:
  ...
  template <typename T> std::vector<T> read_all(int32_t carrier_type = T::tag, int64_t end_time = INT64_MAX) {
    std::vector<T> v{};
    while (data_available() and current_frame()->gen_time() < end_time) {
      if (current_frame()->carrier_type() == carrier_type) {
  ...
  template <typename T>
  std::vector<std::pair<yijinjing::types::frame_header, T>> read_header_data(int32_t carrier_type = T::tag,
                                                                             int64_t end_time = INT64_MAX) {
    std::vector<std::pair<yijinjing::types::frame_header, T>> v{};
    while (data_available() and current_frame()->gen_time() < end_time) {
      if (current_frame()->carrier_type() == carrier_type) {
        v.emplace_back(*reinterpret_cast<yijinjing::types::frame_header *>(current_frame()->address()),
  ...
  template <typename T>
  std::vector<std::pair<yijinjing::types::frame_header, T>> read_header_data(const T &, int64_t end_time = INT64_MAX) {
    return read_header_data<T>(T::tag, end_time);
  }

  std::vector<std::pair<yijinjing::types::frame_header, std::vector<uint8_t>>> read_bytes(int32_t carrier_type,
                                                                                          int64_t end_time = INT64_MAX);

  template <typename T>
  std::vector<std::pair<yijinjing::types::frame_header, std::vector<uint8_t>>>
  read_bytes(const T & = {}, int64_t end_time = INT64_MAX) {
    return read_bytes(T::tag, end_time);
  }

  std::vector<yijinjing::types::frame_header> read_headers(int32_t carrier_type, int64_t end_time = INT64_MAX);

  template <typename T>
  std::vector<yijinjing::types::frame_header> read_headers(const T & = {}, int64_t end_time = INT64_MAX) {
