...
HISTORY_STATUS_SCHEMA = "kungfu.exit-history.status/v1"
HISTORY_EXPORT_OBSERVER_SCHEMA = "kungfu.exit-history.export-observer/v1"
HISTORY_REBUILD_SCHEMA = "kungfu.exit-history.projection-rebuild/v1"
...
def _describe(kind: str, material: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if kind == "profile-source-v1":
        ...
        verified = _product_history_verify(material)
        identity_root = _normalized_root(
...
def _build_material(
    ...
    *,
    ...
    if kind == "product-release-cut-v1":
        try:
            return _product_history_build(config_home or runtime_dir, mode=mode)
        except ProductReleaseHistoryError as error:
...
def _apply_member(
    ...
    *,
    ...
    if kind == "product-release-cut-v1":
        try:
            return _product_history_import(
...
def _history_export_observer_path(runtime_dir: str | Path) -> Path:
    return Path(runtime_dir).expanduser() / "exit" / "history-export-observer.json"


def record_verified_history_export(
    ...
    manifest = package["manifest"]
    body = {
        "schema": HISTORY_EXPORT_OBSERVER_SCHEMA,
        "bundleId": manifest["bundleId"],
        "mode": manifest["mode"],
        "bundleRoot": manifest["bundleRoot"],
        "packageRoot": package["packageRoot"],
    ...
    body["observerRoot"] = _root("history-export-observer", body)
    path = _history_export_observer_path(runtime_dir)
...
def _read_verified_history_export(runtime_dir: str | Path) -> dict[str, Any] | None:
    path = _history_export_observer_path(runtime_dir)
    if not path.is_file():
        return None
    try:
    ...
    if (
        ...
        or body.get("schema") != HISTORY_EXPORT_OBSERVER_SCHEMA
    ):
        raise ExitBundleError(
            "history-export-observer-invalid",
...
def history_status(
    ...
    *,
    ...
    last_export = _read_verified_history_export(runtime_dir)
    body: dict[str, Any] = {
        "schema": HISTORY_STATUS_SCHEMA,
        "ok": True,
        ...
        "contractRoot": _schema_root(contract),
    ...
    if request is not None:
        ...
        package = build(runtime_dir, thin_request, config_home=config_home)
        inspection = inspect(package)
        manifest = package["manifest"]
        body.update(
            {
    ...
    body["statusRoot"] = _root("history-status", body)
    return body


def rebuild_history_projections(
    runtime_dir: str | Path,
    *,
    ...
    if not execute:
        body["receiptRoot"] = _root("history-projection-rebuild", body)
        return body
    actor = authorized_by.strip()
    if not actor:
        raise ExitBundleError(
...
# Project Cut member adapter. Project Cut roots remain domain-owned; the Exit
# composer only invokes this private verifier and exact tracked-byte importer.
...
def _project_cut_build_bundle(
    options: Mapping[str, Any], *, mode: str
) -> dict[str, Any]:
    ...
    return _project_cut_verify_bundle(bundle)
...
def _project_cut_verify_bundle(bundle: Mapping[str, Any]) -> dict[str, Any]:
...
def _project_cut_import_bundle(
    ...
    verified = _project_cut_verify_bundle(bundle)
...
def _product_history_build(
    config_home: str | Path, *, mode: str = "full"
) -> dict[str, Any]:
...
def _product_history_verify(
    bundle: Mapping[str, Any], *, require_full: bool = False
) -> dict[str, Any]:
...
def _product_history_import(
    ...
    *,
    ...
    report = _product_history_verify(bundle, require_full=True)
    if not execute:
        return {
            "ok": True,
            "status": "validated",
