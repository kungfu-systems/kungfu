...
# Preserve `from kungfu.coordination import locks` for existing consumers.
locks = SimpleNamespace(
    **{
        name: globals()[name]
        for name in "acquire held release replace_file status table_path try_acquire with_lock write_json".split()
    }
)
