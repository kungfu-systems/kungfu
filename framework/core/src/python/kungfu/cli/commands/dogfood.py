...
@dogfood.command(
    name="import-atlas", help="plan or execute lossless Atlas JSONL import"
)
@click.argument(
    "source_file", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@_identity_options
@click.option("--expected-source-root", default="")
@click.option("--execute", is_flag=True)
@click.option("--authorized-by", default="")
@dogfood_context
@surface(id="kungfu.dogfood.import-atlas")
@_guard
def import_atlas(
    ctx,
    source_file,
    ...
    if not execute:
        _emit(
            dogfood_api.read(
                runtime_dir, "migration-plan", {"sourcePath": str(source_file)}
            )
        )
        return
    if not authorized_by:
        raise ValueError("--authorized-by is required with --execute")
    dogfood_api.ensure_profile(runtime_dir, authorized_by)
    _emit(
        dogfood_api.action(
            runtime_dir,
            "import-atlas-jsonl",
            {
                ...
                "expectedSourceRoot": expected_source_root,
                "actor": authorized_by,
            },
            authorized_by,
        )
...
@dogfood.command(
    name="export-atlas", help="verify imported roots against retained source bytes"
)
@click.argument(
    "source_file", type=click.Path(exists=True, dir_okay=False, path_type=Path)
)
@_identity_options
@dogfood_context
@surface(id="kungfu.dogfood.export-atlas")
@_guard
def export_atlas(ctx, source_file, workspace_root, home):
    del ctx
    _, runtime_dir, _ = _runtime(workspace_root, home, write=False)
    result = dogfood_api.read(
        runtime_dir, "migration-verify", {"sourcePath": str(source_file)}
    )
    _emit(result)
    if not result["ok"]:
        raise click.exceptions.Exit(4)
