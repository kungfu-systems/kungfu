...
from .local_state import LocalAssignmentStateMixin
from .recovery import AssignmentRuntimeRecoveryMixin


class EmbeddedLocalAssignmentRuntime(
    LocalAssignmentStateMixin, AssignmentRuntimeRecoveryMixin
):
    ...
    def _handle(self, request: Mapping[str, Any]) -> dict[str, Any]:
        ...
        try:
            selected = self._prepare_request(request_value)
            return self._dispatch_operation(request_value, selected)
    ...
    def _prepare_request(self, request: Mapping[str, Any]) -> list[str]:
        self._validate_request(request)
        self._check_realm(dict(request["realm"]))
        selected = self._negotiate(dict(request["client"]))
    ...
    def _dispatch_operation(
        ...
        if operation in {
            ...
            return self._read(request, selected)
        if operation == "events.watch":
            return self._watch(request, selected)
        if operation == "command.submit":
            return self._submit(request, selected)
        if operation == "command.get":
            return self._command_get(request, selected)
        if operation == "diagnostics.get":
            return self._diagnostics(request, selected)
        if operation == "recovery.plan":
            return self._recovery_plan(request, selected)
        if operation == "recovery.execute":
            return self._recovery_execute(request, selected)
        raise LocalRuntimeError(
