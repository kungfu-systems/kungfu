...
json issue_runtime_warrant(const json &descriptor, const lifecycle_view &lifecycle, const json &request,
...
json kfd10_runtime_witness(const json &request, const std::string &runtime_dir) {
  const auto package_key = required_text(request, "packageKey", "request");
  const auto host = required_text(request, "host", "request");
  ...
  const auto mutation = lifecycle_history(runtime_dir, {{"packageKey", package_key}});
