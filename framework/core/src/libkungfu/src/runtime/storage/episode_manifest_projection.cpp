...
nlohmann::json episode_manifest_projection::verify() const {
  ...
  if (!query_records_match) {
    drift.push_back({{"table", QUERY_RECORDS_TABLE},
                     {"projection_rows", projected_records.size()},
                     {"journal_records", records.size()},
                     {"reason", "append-preserving record stream mismatch"}});
  }
  ...
  return {
      ...
      {"query_records", projected_records.size()},
      {"journal_distinct",
