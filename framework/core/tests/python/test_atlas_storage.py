...
def test_mission_control_completion_review_deduplicates_one_subject_across_authorities(
    tmp_path,
):
    runtime_dir = tmp_path / "runtime"
    _activate_mission_profile(runtime_dir)
    ...
    state = mission_control.query_state(
        str(runtime_dir), mission_id="agent-mission", storage_source_id="kungfu"
    )
    matching = [
        row
