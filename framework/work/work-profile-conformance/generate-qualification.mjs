...
const BUILDCHAIN_GATE =
  'framework/work/work-profile-conformance/qualification/kfd-7-product-gate.json';
const BUILDCHAIN_AUTHORITY = '.buildchain/alpha-contract-lock.json';
const ACTION_LOOP_CONTRACT = 'framework/work/action/action-loop.contract.json';
const ACTION_LOOP_FIXTURES = 'framework/work/action/action-loop-fixtures.json';
const ACTION_LOOP_QUALIFICATION_ADAPTERS =
  'framework/work/action/action-loop-qualification-adapters.mjs';
const ACTION_LOOP_BEGIN = 'framework/work/action/action-loop-begin.mjs';
const ACTION_LOOP_SETTLE = 'framework/work/action/action-loop-settle.mjs';
const ACTION_GEOMETRY = 'framework/work/action/action-geometry.contract.json';
const WORK_LIFECYCLE =
  'framework/work/work-lifecycle/kungfu-work-lifecycle-operation-matrix.contract.json';
const WORK_API = 'framework/work/work-loop/work-api.contract.json';
const PLATFORMS = {
...
const KFD_BEHAVIOR = {
  'provider-switch': {
    path: 'framework/work/agent-work/evidence/kfd-7/backend-migration.json',
  ...
  'projection-rebuild': {
    path: 'framework/work/agent-work/evidence/kfd-7/export-import-rebuild.json',
  ...
  recovery: {
    path: 'framework/work/agent-work/evidence/kfd-7/cold-start-continuation.json',
  ...
  'agent-work-control': {
    ...
    roleSchemaPaths: Object.fromEntries(
      ['fact', 'episode', 'pursuit', 'atlas', 'warrant'].map((role) => [
        role,
        `framework/work/agent-work/role-schemas/${role}.schema.json`,
      ]),
    ),
  },
  'week-day-action': {
    domainProfilePath:
      'framework/work/work-profile-conformance/qualification/week-day-domain-profile.json',
    sourcePath:
      'framework/work/work-profile-conformance/qualification/week-day-domain-profile.json',
    roleSchemaPaths: Object.fromEntries(
      ['fact', 'episode', 'pursuit', 'atlas', 'warrant'].map((role) => [
        role,
        `framework/work/work-profile-conformance/qualification/week-day-role-schemas/${role}.schema.json`,
      ]),
    ),
  },
  'course-production': {
...
async function executeActionLoopWitnesses() {
  ...
  const implementation = [
    coordinate('framework/work/action/action-loop.mjs'),
