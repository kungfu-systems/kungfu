...
export function ProjectWorkReviewConfirmation({
  plan,
  busy = false,
  onConfirm,
  onCancel,
  ...
  return (
    <dialog
      open
      aria-modal="true"
      aria-label="Confirm independent review"
      style={{
        position: 'fixed',
        inset: 0,
        zIndex: 1200,
    ...
    >
      ...
      >
        ...
        <div style={{ ...mono, color: '#d7dde5', marginTop: 4 }}>
          {plan.reviewer.label} ·{' '}
          {plan.reviewer.freshProcess
            ? 'fresh read-only process'
            : 'retained passing review'}
        </div>
        <div style={{ ...mono, color: '#aeb8c4', marginTop: 4 }}>
          {plan.deliverable.path}
        </div>
        {plan.executable ? (
          <>
            <p style={{ marginBottom: 6 }}>
              The reviewer will check every acceptance criterion, then retain
              the native review result:
            </p>
            <ol style={{ marginTop: 0, paddingLeft: 24, color: '#e6edf3' }}>
              {plan.effects.map((effect) => (
                <li key={`${effect.stage}:${effect.label}`}>{effect.label}</li>
          ...
          <p style={{ ...mono, color: '#f48771' }}>
            {plan.reviewer.verification.error ||
              'Reviewer or native admission binding did not verify.'}
          </p>
        )}
        <p style={{ ...mono, color: '#b8c2cc' }}>
          No prior transcript is shared. Git commit, push, publication, and
          continuation decisions are excluded. The exact plan is{' '}
...
export function ProjectWorkRunSession({
  ...
  const receipt = run.reviewReceipt ?? run.receipt;
  const status = live
    ? `${spinner} ${run.kind === 'review' ? 'REVIEWING' : 'RUNNING'} · ${elapsedLabel(now - run.startedAt)}${
        quietMs >= 5000 ? ` · waiting ${elapsedLabel(quietMs)}` : ''
      }`
    : run.reviewReceipt
      ? run.reviewReceipt.status === 'review-passed'
        ? 'REVIEW PASSED · SETTLEMENT READY'
        : run.reviewReceipt.status.toUpperCase().replaceAll('-', ' ')
      : run.receipt
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 10, overflow: 'auto', flex: 1 }}>
        ...
        {receipt ? (
          <div
            style={{
              ...mono,
              color: receipt.ok ? '#89d185' : '#f48771',
              marginTop: 8,
            }}
          >
            {run.reviewReceipt
              ...
              : 'Agent run retained'}{' '}
            · Work is {receipt.workPhase}.{' '}
            {receipt.nextActions[0] || 'Inspect the retained evidence.'}
          </div>
        ) : null}
        {run.error ? (
      ...
      {!live &&
      !attention &&
      run.receipt?.status === 'agent-finished' &&
      onReview ? (
        <footer
          style={{
        ...
        >
          <span style={{ ...mono, color: '#cccccc' }}>
            A fresh read-only Agent must verify the retained evidence.
          </span>
          <button type="button" style={actionStyle} onClick={onReview}>
            Review changes
          </button>
