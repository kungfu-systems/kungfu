...
test('workflows keep candidate and promotion outside untrusted PR authority', () => {
  ...
  assert.match(
    promotionWorkflow,
    /promote:[\s\S]*github\.ref_protected == true[\s\S]*event=merge_group[\s\S]*head_sha="\$GITHUB_SHA"[\s\S]*affected-native \/ linux[\s\S]*Multiple Qualified Core producer authorities[\s\S]*framework\/release\/qualified-assignment-core-artifact\.mjs promote[\s\S]*--protected-ref "\$GITHUB_REF"[\s\S]*--delivery-attempt/u,
  );
  assert.match(
    promotionWorkflow,
