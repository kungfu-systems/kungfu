...
export const CPP_STRING_LITERAL_CHUNK_BYTES = 12_000;
...
export function renderCppRawStringChunks(
  payload,
  maxBytes = CPP_STRING_LITERAL_CHUNK_BYTES,
) {
  if (!Number.isSafeInteger(maxBytes) || maxBytes < 1) {
    throw new Error('C++ string literal chunk size must be a positive integer');
  }
  const chunks = [];
  let chunk = '';
  let chunkBytes = 0;
  for (const character of payload) {
    ...
    if (characterBytes > maxBytes) {
      throw new Error(
        'C++ string literal chunk size cannot hold one character',
      );
    }
  ...
  if (chunk || payload.length === 0) chunks.push(chunk);
  return chunks.map((value, index) => {
    const delimiter = `KFPC${index}`;
    if (value.includes(`)${delimiter}"`)) {
      throw new Error(`C++ raw string delimiter collision: ${delimiter}`);
    }
...
export function renderHeader(catalog) {
  ...
  const literals = renderCppRawStringChunks(payload).join('\n');
