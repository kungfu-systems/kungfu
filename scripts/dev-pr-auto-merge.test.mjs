...
test('Qualified native proof re-runs the exact failed source jobs before landing', () => {
  ...
  assert.match(
    landing,
    /uses: kungfu-systems\/buildchain\/\.github\/workflows\/dev-pr-auto-merge\.yml@a7b5144bc1b0a50ef2637dfab23588b3f10ba8ab/u,
  );
...
test('Dev behind admission produces and forwards an exact Project Cut replay proof', () => {
  ...
  assert.match(
    workflow,
    /Check out exact Buildchain delivery runtime[\s\S]*ref: a7b5144bc1b0a50ef2637dfab23588b3f10ba8ab/u,
  );
  assert.match(
    workflow,
...
test('native execution uses one exact protected runtime and continuous fence wrapper', () => {
  ...
  assert.match(action, /ref: a7b5144bc1b0a50ef2637dfab23588b3f10ba8ab/u);
  assert.match(
    action,
    /test "\$\(git rev-parse HEAD\)" = a7b5144bc1b0a50ef2637dfab23588b3f10ba8ab/u,
  );
  assert.match(
    action,
  ...
  assert.match(
    workflow,
    /\.\/shifu dev-delivery:native-under-warrant[\s\S]*--credential-ancestry-boundary github-actions-runner-worker\/v1[\s\S]*--runner-environment github-hosted/u,
  );
  assert.match(
    fs.readFileSync('framework/dev-delivery/native-under-warrant.mjs', 'utf8'),
    /flag\([\s\S]*'credential-ancestry-boundary'[\s\S]*BUILDCHAIN_CREDENTIAL_ANCESTRY_BOUNDARY[\s\S]*flag\([\s\S]*'runner-environment'[\s\S]*BUILDCHAIN_RUNNER_ENVIRONMENT/u,
  );
});
