...
test('Dev auto-merge admits only explicitly ready reviewed source-bound PRs', () => {
  ...
  assert.equal(reusableRef, '7715df884efd4da1cc1ceceb007609c217b4e169');
...
test('Qualified native proof re-runs the exact failed source jobs before landing', () => {
  ...
  assert.match(
    landing,
    /uses: kungfu-systems\/buildchain\/\.github\/workflows\/dev-pr-auto-merge\.yml@7715df884efd4da1cc1ceceb007609c217b4e169/u,
  );
...
test('Dev behind admission produces and forwards an exact Project Cut replay proof', () => {
  ...
  assert.match(
    workflow,
    /Check out exact Buildchain delivery runtime[\s\S]*ref: 7715df884efd4da1cc1ceceb007609c217b4e169/u,
  );
  assert.match(
    workflow,
