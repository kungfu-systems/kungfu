...
test('Dev auto-merge admits only explicitly ready reviewed source-bound PRs', () => {
  ...
  assert.equal(reusableRef, '14d5d5a4a3e8865f7acb61abb0cfc36ed734aad5');
...
test('Qualified native proof re-runs the exact failed source jobs before landing', () => {
  ...
  assert.match(
    landing,
    /uses: kungfu-systems\/buildchain\/\.github\/workflows\/dev-pr-auto-merge\.yml@14d5d5a4a3e8865f7acb61abb0cfc36ed734aad5/u,
  );
...
test('Dev behind admission produces and forwards an exact Project Cut replay proof', () => {
  ...
  assert.match(
    workflow,
    /Check out exact Buildchain delivery runtime[\s\S]*ref: 14d5d5a4a3e8865f7acb61abb0cfc36ed734aad5/u,
  );
  assert.match(
    workflow,
