...
test('Dev behind admission produces and forwards an exact Project Cut replay proof', () => {
  ...
  assert.match(
    workflow,
    /Check out exact Buildchain delivery runtime[\s\S]*ref: ddac1876650f2faba5a9e1a2e5f55e7d836ac46b/u,
  );
  assert.match(
    workflow,
...
test('native execution uses one exact protected runtime and continuous fence wrapper', () => {
  ...
  assert.match(action, /ref: ddac1876650f2faba5a9e1a2e5f55e7d836ac46b/u);
  assert.match(
    action,
    /test "\$\(git rev-parse HEAD\)" = ddac1876650f2faba5a9e1a2e5f55e7d836ac46b/u,
  );
  assert.match(
    action,
