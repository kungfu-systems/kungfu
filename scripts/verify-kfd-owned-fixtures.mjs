...
function verifyCurrentAtlas(xinfaBinary, objectPath) {
  const receipt = JSON.parse(
    run(xinfaBinary, ['atlas', 'verify', '--atlas', objectPath, '--json']),
  );
  assert.equal(
    receipt.valid,
    true,
    `Xinfa rejected the current protected Atlas: ${JSON.stringify(receipt.diagnostics)}`,
  );
...
try {
  ...
  const profiles = {
    ...
    atlas: verifyCurrentAtlas(xinfaBinary, atlasOutput),
