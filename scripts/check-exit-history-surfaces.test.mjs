...
test('all history verbs delegate to the existing Core authority seams', () => {
  assert.match(cli, /exit_bundle\.history_status/u);
  ...
  assert.match(cli, /exit_bundle\.rebuild_history_projections/u);
  assert.match(core, /memberDomainsRemainAuthoritative/u);
  assert.match(core, /observerMutation": False/u);
  assert.match(core, /authorityMutation": False/u);
  assert.doesNotMatch(core, /shutil\.copy.*projection/u);
});

test('GUI and TUI observe the same status and exact next action', () => {
  for (const source of [gui, tui]) {
    assert.match(source, /\['exit', 'history', 'status', '--json'\]/u);
    assert.match(source, /historyStatus\.nextActions\[0\]/u);
    assert.match(source, /EXIT_HISTORY_STATUS_FALLBACK/u);
    assert.match(source, /kungfu exit history status --json/u);
  }
