...
| CLI `next_actions` and gate projection | `framework/core/src/python/kungfu/assignment_orchestration.py::next_actions` and `gate` | Client-side orchestration derives next actions from the Profile-owned phase | Runtime snapshot carries canonical next actions and gate receipts | centralize in R1; remove duplicate fold only after parity |
