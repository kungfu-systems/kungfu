...
| Runtime | Stable release authority is `@kungfu-tech/buildchain@3.0.0`; dev/MQ telemetry uses the isolated npm alias `@kungfu-tech/buildchain-alpha@npm:@kungfu-tech/buildchain@3.0.2-alpha.2`. `alpha` binds the protected v3 runtime at `f288ea1ab6a69fa6085f7616261fa5a6ba60f671`, which adds Linux GitHub Artifact Attestation and retains sanitized Apple notarization rejection evidence without changing the compatible v3 contract, while `release` remains isolated on `v3` at `9e904de2c85dbea7c799780ee166510b3336d812`; each channel retains its exact contract lock and digest, and the standalone Shifu tool pin is tracked separately in `.buildchain-version` |
