...
| Runtime | Stable release authority is `@kungfu-tech/buildchain@3.0.0`; dev/MQ telemetry uses the isolated npm alias `@kungfu-tech/buildchain-alpha@npm:@kungfu-tech/buildchain@3.0.2-alpha.2`. `alpha` binds the protected v3 runtime at `2123ee9c76013f09888d3a01543e66762a6b5819` (`v3.0.2-alpha.5`), which combines the auditable Demo artifact Gate and Linux GitHub Artifact Attestation without changing the compatible v3 surface, while `release` remains isolated on `v3` at `9e904de2c85dbea7c799780ee166510b3336d812`; each channel retains its exact contract lock and digest, and the standalone Shifu tool pin is tracked separately in `.buildchain-version` |
