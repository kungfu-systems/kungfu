...
| Runtime | Stable release authority is `@kungfu-tech/buildchain@3.0.0`; dev/MQ telemetry uses the isolated npm alias `@kungfu-tech/buildchain-alpha@npm:@kungfu-tech/buildchain@3.0.2-alpha.2`. `alpha` explicitly accepts the protected artifact-signing authority at `2522e79e4c00233a5d15f887360547ed1034c39e`: its auditable Demo surface binds regenerated pull-merge evidence to the exact artifact coordinate, its native matrix independently routes offline self-hosted Linux, macOS, and Windows lanes to fixed GitHub-hosted images, and requested binary artifacts are signed and notarized inside Buildchain's central credential island before the exact final bytes are verified on GitHub-hosted infrastructure. `release` remains isolated on `v3` at `9e904de2c85dbea7c799780ee166510b3336d812`; each channel retains its exact contract lock and digest, and the standalone Shifu tool pin is tracked separately in `.buildchain-version` |
