...
| Runtime | Consumer tooling uses production `@kungfu-tech/buildchain@3.0.6`. Workflow sources remain the deployed floating v3 contracts: `v3` resolves to `380b2d8c2a660b07ed785e71276f71dc6a9184f7`, while Alpha routing selects `v3-alpha` at `dd702f22e6afef137c86c5456e167e6db20e88f2` and the bounded promotion shell at `1d9b35b75e16d492e6d8b18c32504e89f8806855`. The source never exact-SHA pins those workflows; the contract locks retain their actual resolved digests as immutable evidence. Manual diagnostic builds may pass one temporary train or exact runtime SHA through `workflow_dispatch`; the production `@v3` shell independently restricts that override to trusted actors, resolves it before matrix work, and records both shell and runtime identities. Pull-request and channel builds retain the empty-input `v3` default. Buildchain keeps full-history ancestry visible, binds the source attempt and artifact coordinates, runs pre-upload transport smoke, and signs/notarizes declared product artifacts through its credential island. Consumer repositories declare artifacts only; they do not own certificate environments, secrets, or signing jobs. The standalone Shifu tool pin remains tracked separately in `.buildchain-version`. |
