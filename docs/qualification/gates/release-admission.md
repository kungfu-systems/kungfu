...
| Runtime | Alpha consumer tooling uses `@kungfu-tech/buildchain@3.0.9-alpha.2`; stable admission remains isolated behind `@kungfu-tech/buildchain-stable@3.0.7`. Workflow routing keeps the deployed floating contracts: stable uses `v3` at `81c49578bff72a9784a1b63ce8698bd9dd23d7bf`, while Alpha selects `v3-alpha` at `e5e7078c67978051016c2ceb1fec114e8956397a`; sealed-candidate recovery binds that reviewed Alpha publication runtime exactly. Candidate build sources retain the floating channel contract rather than a committed exact-SHA override, and the contract locks record the actual resolved digest as immutable evidence. Buildchain keeps full-history ancestry visible, binds source and artifact coordinates, performs the pre-upload transport smoke, and signs or notarizes declared product artifacts through its credential island. Consumer repositories declare artifacts only; they do not own certificate environments, secrets, or signing jobs. The standalone Shifu tool pin remains tracked separately in `.buildchain-version`. |
