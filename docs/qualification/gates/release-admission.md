...
| Runtime | Stable release authority is `@kungfu-tech/buildchain@3.0.0`; dev/MQ telemetry uses the isolated npm alias `@kungfu-tech/buildchain-alpha@npm:@kungfu-tech/buildchain@3.0.2-alpha.2`. `alpha` binds the protected v3 runtime at `c83cb444d79c2e647de6e12a0bfde68dfaf57701` (`v3.0.2-alpha.7`), which combines the auditable Demo artifact Gate, Linux GitHub Artifact Attestation, and sanitized Apple notarization rejection evidence without changing the compatible v3 surface, while `release` remains isolated on `v3` at `9e904de2c85dbea7c799780ee166510b3336d812`; each channel retains its exact contract lock and digest, and the standalone Shifu tool pin is tracked separately in `.buildchain-version` |
