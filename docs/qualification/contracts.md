...
Python, Node, and the assembled product read that contract rather than carrying
...
`verify` checks that the assembled artifact copy under
`framework/core/dist/kungfu/config/` has the same SHA-256 as the repo contract,
then confirms the assembled runtime reports that same hash through
`kungfu config show --json`.
...
Python CLI (`kungfu kfx`), Skill dependency binding, and assembled products read
...
`verify` checks that the assembled artifact copy under
`framework/core/dist/kungfu/config/` has the same SHA-256 as the repo contract,
then confirms the assembled runtime reports the same hash through
`kungfu kfx contract --json`.
The living welded-surface register is [`versioning.md`](../development/versioning.md), surface
`kfx-contract`.
...
[`kungfu-contracts.registry.json`](../../framework/spec/contract/kungfu-contracts.registry.json).
Build, assembly, and verify use that registry rather than separate hard-coded
contract lists. The assembled runtime ships the registry under
...
[`kungfu-agent-first-canonical-policy.json`](../../framework/spec/contract/kungfu-agent-first-canonical-policy.json).
...
recipes, and the assembled artifact paths. The standard key, schema ids, and JSON
...
**Guarantee.** The
[`kungfu-data-protection.contract.json`](../../framework/core/data-protection/kungfu-data-protection.contract.json)
...
Read-only checks preserve the source SHA recorded by the committed KFD-3
prebuild witness and require it to be an ancestor of the checked source, to have
an exact first-parent tree-equivalent ancestor after a protected rebase, or to
have an exact cumulative Project Cut patch replay on the advanced protected
base. For GitHub's synthetic two-parent pull-request merge, the checker may
search the candidate parent's bounded first-parent history only when that
parent's tree is exactly the checked merge tree. Patch replay equivalence
requires the complete binary, full-index, no-rename diff from the original
merge base to the witnessed source to equal the complete diff from the checked
protected base to one candidate ancestor. Merges that alter candidate content,
omit the protected base, or only partially replay the cut remain ineligible.
...
Node managers validate their generated outputs against those schemas. Assembled
...
`verify` checks the assembled Skill contract artifact and smoke-tests that the
assembled runtime reports the same contract hash through
`kungfu skill contract --json`.
The living welded-surface register is [`versioning.md`](../development/versioning.md), surface
`skill-contract`.
