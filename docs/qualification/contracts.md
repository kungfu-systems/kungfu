...
prebuild witness and require it to be an ancestor of the checked source (or to
have an exact first-parent tree-equivalent ancestor after a protected rebase).
For GitHub's synthetic two-parent pull-request merge, the checker may search the
candidate parent's first-parent history only when that parent's tree is exactly
the checked merge tree; merges that alter candidate content remain ineligible.
