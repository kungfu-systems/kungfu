...
| Provider authority and migration | Atomic backend binding generation in `libkungfu` | File↔RocksDB copy, cross-process shared/exclusive write fence, semantic-root verification, resumable state, retained-provider rollback, and Python/Node/CLI receipts | KF-ADR-019f86da-4f90-713b-a44c-0677d2446cc1; `test_storage_backend_switch.py`; Node authority-atomic binding test | Single-host operation/authority locks; no cross-machine consensus or destructive source cleanup |
...
| Fact admission | KFD-1 declaration plus journaled admission history | Initial declaration, observation, admission, correction/retraction, and historical query path implemented | KF-ADR-019f86da-4f90-7d81-90a0-d144fc27fe03; `fact_admission.h`; `test_atlas_storage.py` | Admission is not universal external truth |
| Fact query | C++ query basis, logical plan, authority scan, and lineage | Implemented staged query surface over pinned declarations and cuts | `fact_query.h`; KF-ADR-019f86da-4f90-7e38-b72f-ef8829e14104 tests | Broader SQL/distributed query qualification is separate |
...
| Durability | Typed facts and named KF-ADR-019f86da-4f90-7ec5-a83c-99cfaee56aca profiles | Default-off current-hardware candidate evidence exists | durability qualification and retained evidence | Physical power loss and production eligibility remain false |
...
- KF-ADR-019f86da-4f90-70c5-b572-89ec183b37de is `accepted` and `staged`: its provider-neutral service, typed
...
- KF-ADR-019f86da-4f90-7828-9142-46f9bca4b0f5 is `accepted` and `implemented`: source and manifest catalog records
  are typed journal authority; JSON and SQLite are projections.
- KF-ADR-019f86da-4f90-738c-b372-e509976f69ff is `accepted` and `implemented` for its embedded first delivery.
  ...
  tag.
- KF-ADR-019f86da-4f90-713b-a44c-0677d2446cc1 is `accepted` and `staged`: the binding, resumable bidirectional
...
- KF-ADR-019f86da-4f90-7d81-90a0-d144fc27fe03 is `accepted` and `implemented` for the initial KFD-1 declaration and
  admission path; broader domain scaffolding remains incremental.
- KF-ADR-019f86da-4f90-7ec5-a83c-99cfaee56aca remains `accepted` and `staged`; its evidence boundary is authoritative
