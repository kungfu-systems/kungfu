...
| **action envelope** | The FlatBuffers open-layer envelope carried by the generic action carrier. `action_type` and `schema_ref` hold business meaning; JSON is only an edge rendering/interchange form. The current pre-release JSON/base64 implementation is transitional under ADR-0047. |
