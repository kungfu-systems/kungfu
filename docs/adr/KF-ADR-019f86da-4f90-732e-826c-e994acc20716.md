...
qualification_refs: [framework/core/tests/python/test_work_control_profile.py, framework/core/tests/python/test_agent_console_contract.py, extensions/work-dashboard/tests/work-control-profile.test.ts, framework/core/src/python/kungfu/agent/kfd3_api.registry.json, framework/evidence/schema/evidence-envelope-v1.schema.json, framework/evidence/src/evidence-envelope.mjs, scripts/check-evidence-envelope.test.mjs, framework/project-cut/qualification/three-agent-dogfood/final-actor-b-review.json, framework/project-cut/qualification/three-agent-dogfood/final-actor-c-continuation.json, framework/project-cut/qualification/three-agent-dogfood/final-phase-matrix.json]
...
the context bus from one person to another. The handoff must instead pin the Assignment
...
`claim-completion` records the claimant and Assignment set together with acceptance,
...
equal the claim commit and cross-checks the commit tree, Assignment acceptance and
...
Assignment rows with explicit dependencies, acceptance root, and `why_created`.

An agent may materialize only mechanical follow-ups. Initiative, authority,
...
- a valid mechanical plan creates only its bounded follow-up Assignment rows; and
...
chat, and a third agent can continue from the resulting Assignment plan. The additional
