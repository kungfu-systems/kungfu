...
- Subsystem: runtime storage service, source registry, manifest-backed import
  and export, `location` / `channel` runtime concepts, external source adapters, and
...
An external-source import exposes the same shape. The external system remains
the authority, while Kungfu verifies imported local payloads against declared
source roots. This is intentionally
...
A source is a logical registry entry: a local profile, imported bundle, external
...
- source import is the first source-adapter proof, not a one-off migration path.
...
```text
external source -> Kungfu local storage
```

External source remains the source of truth. Kungfu imports declared facts, stores large
payloads behind hash-addressed references, writes a manifest, runs fsck, exports
canonical JSONL, and verifies imported hashes against the consumer repository.
