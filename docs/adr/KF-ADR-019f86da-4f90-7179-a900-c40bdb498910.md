...
# KF-ADR-019f86da-4f90-7179-a900-c40bdb498910: yijinjing journal frame/page publish protocol → `atomic_ref` release/acquire
...
- Related: independent of schema ownership (historical KF-ADR-019f86da-4f90-7a55-9b15-93fcab44a33a, current
  KF-ADR-019f86da-4f90-71eb-b4c0-376ca7bc7ad3); this change touches only publish synchronization, not schema or
  on-disk layout
...
3. `writer.cpp`:
   - frame commit moves the `length` publish to **after** gen_time / frame_uid /
     trigger_frame_uid / last_frame_position / next-header zeroing are all done,
     using `publish_data_length()` (release) as the finalizer;
...
- **Next directions**: ① whether to give the same scrutiny to the
  `close_page` / page-switch path's visibility; ② whether the KF-ADR-019f86da-4f90-7a55-9b15-93fcab44a33a born-FB
...
- `framework/core/src/libyijinjing/include/kungfu/yijinjing/schema/types.h` (frame_header: drop volatile)
- `framework/core/src/libyijinjing/include/kungfu/yijinjing/journal/frame.h` (acquire/publish/copy)
- `framework/core/src/libyijinjing/src/journal/writer.cpp` (transaction commit / copy_frame)
