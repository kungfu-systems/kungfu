...
- Status: accepted (criterion + direction); all three decisions landed — the line
  ...
  Rewind decode plus domain checksum consumers call the exposed primitives) — and the
...
- **Outer rings re-implemented those generic primitives.** `rewind`'s Python
  ...
  decoder and a retired domain checksum consumer duplicated the C++ checksums
  and **hard-coded the `frame_header` field layout**. Both duplications have
  been removed.
...
- Rewind (agent execution causal trees), Work (Assignment projections), Profile
  (composition / validation / planning), and Work Control (Initiative and
  Assignment projection assembly / scoring). These are domains owned above the
  generic storage membrane.
...
- Domain checksum consumers call the exposed C++ `checksum_frame` /
  `checksum_payload`; no domain-owned checksum implementation remains.
...
- **Positive**: outer rings stop re-implementing generic primitives; non-C++ consumers reach
...
- **Explicit non-goals**: do **not** migrate domain folds (Rewind / Work /
  Profile / Work Control) or `flatc`-generated domain accessors
  into libkungfu — that is correct placement per KF-ADR-019f86da-4f90-7c91-9cc2-6dbd18d68dff, not debt.

## Alternatives considered

1. **Migrate the domain folds into libkungfu.**
   Rejected: bloats the core, defeats the membrane's purpose, and violates
   KF-ADR-019f86da-4f90-7c91-9cc2-6dbd18d68dff — agent and Work semantics are *domains*, which the application owns, not the
   generic core.
2. **Leave the Python re-implementations as they are.** Rejected: a DRY and
   ...
   the retired checksum implementation hard-coded the `frame_header` layout; and
...
- **Landed (de-dup delivery, this PR)**: Decision 3 — `rewind` `BundleDecoder`
  ...
  `None` for contract parity) and domain checksum consumers call `checksum_frame` /
