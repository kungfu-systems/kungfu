...
qualification_refs: [framework/core/tests/python/test_storage_cli.py, framework/core/tests/python/test_content_store_facade.py, framework/core/tests/storage-node-binding.test.js]
...
- **FlatBuffers open layer** (`*.fbs` + the `.bfbs` reflection projector):
  domain and replayable payloads (`source_events`, `rewind_events`,
...
5. **JSON is an edge projection only — including at adapter input.** The
   ...
   source (for example an source JSON repository) legitimately parses that source
...
- Project it to SQLite via the Hana → SQLite path; store any body as
  ...
  Python source importer. Body format is orthogonal to the record schema; the
