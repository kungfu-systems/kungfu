...
# KF-ADR-019f86da-4f90-7b50-9cb5-715a82f9e7c4: Codex App Server is a pinned structured-hybrid provider adapter
...
Codex `0.146.0` `app-server --stdio` launch in the reviewed start plan and
...
`0.146.0` attempts. `KUNGFU_AGENT_SESSION_CODEX_APP_SERVER=0` is the explicit
...
Authenticated Mac source dogfood against Codex `0.146.0` passes start,
