...
adr_id: KF-ADR-019f86da-4f90-7b50-9cb5-715a82f9e7c4
decision_status: accepted
implementation_status: staged
implementation_prs: [https://github.com/kungfu-systems/kungfu/pull/849, https://github.com/kungfu-systems/kungfu/pull/851, https://github.com/kungfu-systems/kungfu/pull/855, https://github.com/kungfu-systems/kungfu/pull/857, https://github.com/kungfu-systems/kungfu/pull/861, https://github.com/kungfu-systems/kungfu/pull/2184]
qualification_refs: [framework/agent-session/tests/codex-app-server-contract.test.mjs, framework/agent-session/tests/codex-app-server-schema.native.test.mjs, framework/agent-session/tests/codex-app-server-runtime.test.mjs, framework/agent-session/tests/codex-app-server-interaction.test.mjs, framework/agent-session/tests/codex-app-server-recovery.test.mjs, framework/agent-session/tests/codex-app-server-product.test.mjs, framework/agent-session/schemas/codex-app-server/codex-v0.146.0-stable-schema-manifest.json, framework/agent-session/tests/session-contract.test.mjs, scripts/run-agent-session-provider-dogfood.mjs]
...
ai_provenance: GPT-5 via Codex on 2026-08-13; visible context included the existing ADR, current Agent Session contracts, implementation, qualification tests, and user-directed version-neutral admission boundary; did not inspect provider credentials, private transcripts, hidden sessions, or unreleased provider internals
---

# KF-ADR-019f86da-4f90-7b50-9cb5-715a82f9e7c4: Codex App Server is a schema-qualified structured-hybrid provider adapter

- Status: accepted; implementation staged
- Date: 2026-07-14
- Category: product runtime / provider adapter / structured interaction
- Related: [KF-ADR-019f86da-4f90-7977-9cfc-85d26d41780b](KF-ADR-019f86da-4f90-7977-9cfc-85d26d41780b.md),
...
KF-ADR-019f86da-4f90-7fbf-9949-b7ed353dfaef establishes one provider-neutral Agent Interaction Port and a PTY-owning
...
The surface can evolve across provider releases and does not provide three properties Kungfu
...
KF-ADR-019f86da-4f90-7fbf-9949-b7ed353dfaef. The adapter therefore needs an exact, provider-specific boundary.

## Decision

### 1. Codex structured attempts use direct stdio and schema-qualified capability admission

The production candidate is local `codex app-server --stdio` with:

- any available Codex CLI, with its reported version retained as diagnostic
  metadata only;
- the non-experimental stable schema bundle qualified from Codex CLI `0.146.0`
...
A version mismatch or failed `--version` probe may emit a warning but cannot
close launch admission. Actual schema digest, required field, admitted method,
capability, or deployment drift still fails closed after capability negotiation
and requires targeted requalification. There is no best-effort parser for
unknown methods.
...
The pinned `0.146.0` stable bundle contains 275 files and exposes 90 client
requests, one client notification, ten server requests, and 70 server
...
5. Consume the shared product seam behind a feature flag after KF-ADR-019f86da-4f90-7fbf-9949-b7ed353dfaef product
   surfaces are stable.
...
Stage 5 is delivered by PR #861: an opt-in product adapter initially froze the
Codex `0.146.0` `app-server --stdio` qualification source in the reviewed start plan and
...
attempts. Runtime version metadata does not select or reject the route; the
structured handshake and admitted message schema do. `KUNGFU_AGENT_SESSION_CODEX_APP_SERVER=0` is the explicit
...
Authenticated Mac source dogfood against Codex `0.146.0` is retained as one
historical evidence point and passes start,
...
upgrade that result. It is not a support allowlist for later, earlier, opaque,
or locally patched Codex builds.
