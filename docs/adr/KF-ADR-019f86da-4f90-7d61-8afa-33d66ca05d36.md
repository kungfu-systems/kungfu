...
- Related: KF-ADR-019f86da-4f90-7e58-bb03-bee0f101dc01 defines workspace-local `.kungfu`; KF-ADR-019f86da-4f90-7394-9953-5dbb467859fa defines the
  per-user supervisor and per-data-root coordinator; the native Work Control contract defines Initiative
...
renders Initiative/Assignment lists and authoring forms against whichever runtime home was
inherited.

This breaks the intended product model in two directions. A user should be able
to open a real workspace such as `~/Code/example-project`, return to it on the next
...
work without preparing Markdown, Git, or a Initiative hierarchy first. Opening a
...
- create or clarify a Initiative;
- create a Assignment, claim completion, or record a decision;
- import Atlas or materialize a Initiative/Episode/fact bundle;
...
Read-only Initiative/Assignment inspection, workspace probing, config inspection, bundle
...
Home may begin with an Agent Work Inbox and no Initiative. In
that state Kungfu records what happened and what the evidence establishes, but
reports purpose-bound fitness as insufficient until a Initiative/Assignment or explicit
...
Initiative bodies, Assignment state, facts, proofs, or imported source payloads. A missing
...
| `<project>/.kungfu` | project declarations, admitted facts, Episodes, payloads, source registry, Initiative/Assignment/claim/decision facts, assessments, TrustReports, saved queries, observer metadata, rebuildable projections and coordinator state | global recent workspaces, installed-product preferences, implicit Home facts |
| `~/.kungfu-config` (or `KF_CONFIG_HOME`) | user preferences, global trust/extension/skill policy, installed kfx/skill metadata, recent/last workspace registry, per-user supervisor routing state | workspace Initiative/Assignment facts, saved query revisions, proof or imported Atlas bodies |
| platform machine fallback selected by `KF_HOME` | no-workspace runtime support, caches, service state, explicitly machine-scoped facts | an implicit merged Initiative world for all projects |
...
projections as needed. A completed source import, native Initiative, Assignment, assessment,
or Saved Query Catalog entry is immediately available from the same authority.
Opening does not re-import an external source and does not mutate source authority.

If the selected root looks source-compatible but no completed import exists,
Initiative Home may offer **Import external facts**. The action is explicit and is a
...
resolution reason, and establish no project or Initiative association.

Read-only validation may operate without a workspace. Semantic writes such as
Initiative/Assignment creation may offer Home interactively, but non-interactive/JSON use
...
repository, a long-lived Initiative, or a portability/collaboration request. The
...
- A first-time user can capture and manage agent work before owning a repository
  or fully articulating a Initiative, without the product inventing purpose.
...
- Saved Query Catalog remains correctly workspace-scoped; transfer uses its
  JSON artifact or a larger Initiative/Episode bundle, not implicit global reuse.
...
- **Use Electron `userData/runtime` for every Initiative.** Rejected because it
...
- **Require a repository or Initiative before tracking agent work.** Rejected
...
- Choosing **Start managing agent work** and performing the first managed run
  ...
  unassigned inbox without inventing a Initiative.
...
- Attaching that Episode to a later Initiative preserves its identity and original
  capture boundary.
...
- The first Initiative/Assignment/import/saved-query write creates exactly that
...
- Switching workspace cannot expose the prior workspace's Initiative, query, or
  runtime handles.
...
- Recent workspace paths can be sensitive metadata; the registry remains local,
  bounded, and outside portable Initiative bundles.
