...
adr_id: KF-ADR-019f86da-4f90-73f2-a0ac-42f14e0278d9
...
# KF-ADR-019f86da-4f90-73f2-a0ac-42f14e0278d9: Episode identity is two layers — a local coordinate plus a sealed content root committed in the manifest
...
- Related: KF-ADR-019f86da-4f90-791c-9b90-4888cca36327 defines Episode as the first-class causal segment object
  and requires the manifest to record "hash roots or sync roots needed by
  fsck/export/import"; KF-ADR-019f86da-4f90-762d-a677-5e8984cc6692 puts the manifest records in the yijinjing
  journal; KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae makes the POD journal plus one typed fold the trust
  boundary and defers "deep Episode identity and hash-root composition" to
  this ADR; KF-ADR-019f86da-4f90-7516-b7ed-5b39a527cefb defines atomic safety and explicitly leaves "selecting
  the final Episode id or hash-root composition algorithm" out of scope —
  this ADR closes that deferral. KF-ADR-019f86da-4f90-738c-b372-e509976f69ff provides the content-addressed
  store the root builds on; KF-ADR-019f86da-4f90-7d72-bf9f-1d5913bbb0d5/0028 define frame checksum and content
  hash boundaries. The sync-root linear chain
  (`kungfu.sync-root/v1`) is the composition precedent this ADR follows.

## Context

Every storage decision so far organizes around Episode (KF-ADR-019f86da-4f90-791c-9b90-4888cca36327), and the
manifest is the object's trust boundary (KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae): the journal of POD claims
...
- The manifest records per-frame checksums (KF-ADR-019f86da-4f90-7d72-bf9f-1d5913bbb0d5) and per-ref content
  hashes (KF-ADR-019f86da-4f90-738c-b372-e509976f69ff), so each *individual* claim is checkable, but there is no
...
- Export/import, dedup, and future sync need an equality judgment that
  ...
  exactly the kind of unverifiable coordinate KF-ADR-019f86da-4f90-791c-9b90-4888cca36327 rejects as the user
  fact object.
...
JSON canonicalization inside the trust boundary (KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae's discipline: JSON
is edge-only).
...
- every `EpisodeFrameAttached` (each carries the frame's uid, times,
  carrier/source/dest, length, and KF-ADR-019f86da-4f90-7d72-bf9f-1d5913bbb0d5 checksums — so the root commits
  transitively to frame content);
- every `EpisodeRefAttached` (each carries `ref_id` and `ref_hash` — so the
  root commits transitively to payload bytes through the KF-ADR-019f86da-4f90-738c-b372-e509976f69ff
...
conflicting claims — is the authority for *what was claimed* (KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae fold
...
- A new manifest record type commits the root:
  ...
  ```text
  EpisodeRootCommitted (new carrier type, additive to the KF-ADR-019f86da-4f90-762d-a677-5e8984cc6692 family)
  ...
  KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae required a schema-version ADR before extending the record set;
  ...
  record without folding it (the KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae unknown-record contract).
- **fsck recomputes and verifies.** For a sealed Episode with a root record,
  ...
  cannot canonicalize — honest degradation per KF-ADR-019f86da-4f90-7516-b7ed-5b39a527cefb).
...
- Episodes sealed before this ADR, and seals interrupted between
  ...
  maintenance/repair op may add roots later under KF-ADR-019f86da-4f90-7516-b7ed-5b39a527cefb's
  evidence-preserving rules).
- `advertised_capabilities ⊆ evidence_safe_capabilities` (KF-ADR-019f86da-4f90-7516-b7ed-5b39a527cefb) applies:
...
- A sealed Episode finally *is* something: a verifiable content identity that
  travels with its records, closing the loop KF-ADR-019f86da-4f90-791c-9b90-4888cca36327 opened when it named
...
- **Derived-only root (compute on demand, never record).** Rejected: the
  trust boundary must *record* identity (KF-ADR-019f86da-4f90-791c-9b90-4888cca36327), otherwise export/import
...
- **Reuse `EpisodeRefAttached` with a new ref kind to carry the root.**
  ...
  overloading KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae forbids without a schema ADR — and once writing a
...
- **JSON-canonicalized commitments (reuse sync-root entry hashing as-is).**
  ...
  boundary, the drift KF-ADR-019f86da-4f90-737e-893f-c095b9a05cae exists to eliminate. The chain shape is
