...
sensitivity: public
period: ongoing
theme: frontend-package-consumption
confidence: high
evidence_grade: B
last_reviewed: 2026-09-05
ai_provenance: GPT-6 via Codex on 2026-09-05; package-consumption implementation evidence from checked-in source and local tests; no claim about unseen runtime or public release evidence
qualification_refs: [scripts/check-package-boundaries.mjs, scripts/check-package-boundaries.test.mjs, scripts/qualify-work-package.mjs]
---
...
## Workspace package consumption enforcement (2026-09-05)

The capability SDK, reference surfaces, and extensions now consume sibling
packages through declared dependencies and explicit package exports. Work
protocols use the same boundary. Cross-package relative imports, source aliases,
and constructed source-file build entries are rejected by the source gate.
Public metadata entries preserve SDK package discovery.

This implements the package separation already decided here. It preserves
runtime authority and the existing frontend contract; the overall ADR remains
partial. Verification includes the package-boundary negative tests, reference
GUI and TUI builds, and a clean local-tarball Work consumer that rejects private
implementation imports. Repository tooling exports can still require a checkout
and development dependencies; this is not qualification of every package for
