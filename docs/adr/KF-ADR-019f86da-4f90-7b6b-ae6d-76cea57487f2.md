...
confidence: high
evidence_grade: B
ai_provenance: GPT-5 via Codex on 2026-07-30; this update adds the source-bound, publish-none macOS overflow controller and its contract tests as bounded implementation evidence; no claim is made about future runner capacity or publication qualification
...
after a bounded queue or health decision, binds both candidates to the same
source and preflight, and forces both to `publish-channel: none`. Its
machine-readable receipt records route, queue, runner, cancellation, result,
and winner facts without gaining promotion authority.
