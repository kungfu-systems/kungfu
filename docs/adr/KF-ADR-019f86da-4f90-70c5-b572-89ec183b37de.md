...
implementation_prs: [https://github.com/kungfu-systems/kungfu/pull/459, https://github.com/kungfu-systems/kungfu/pull/476, https://github.com/kungfu-systems/kungfu/pull/480, https://github.com/kungfu-systems/kungfu/pull/485, https://github.com/kungfu-systems/kungfu/pull/1459]
qualification_refs: [framework/core/tests/python/test_storage_cli.py, framework/core/tests/python/test_content_store_facade.py, framework/core/tests/storage-node-binding.test.js, framework/core/tests/core-platform-package.test.js, framework/core/tests/node-api-authority.test.js]
...
- Subsystem: `framework/core` journal runtime, fact-ledger format direction,
  source import storage slice, payload/blob storage, SQLite projections, and the
...
- SQLite projections that are useful for query and UI state but must remain
  rebuildable;
- a first source import slice that stores large JSON payloads outside mmap
  frames, writes manifests, runs `fsck`, exports JSONL, and verifies source
  hashes against the consumer repository.
...
The accepted first delivery is a source-neutral proof surface:

```sh
kungfu storage status --scope source --source <source-id> --json
kungfu storage fsck --scope source --source <source-id> --json
kungfu storage export --scope source --source <source-id> --format jsonl --out source.jsonl --json
```

That slice proves the direction without claiming the whole service is complete:

- Large source payloads are stored outside mmap frames as hash-addressed
  bodies. The default provider stores those payloads as content-addressed
...
- `fsck` detects missing payloads, hash mismatches, malformed payload JSON, and
  projection drift.
- `storage export` emits canonical JSONL records for imported source payloads.
- `storage fsck` recomputes hashes from the accepted source manifest and
  compares them with the imported payload inventory.
...
- Declaring any imported source no longer authoritative.
...
- **Keep import/export tied to one consumer adapter.** Rejected. Consumer
  identity is not storage architecture; the mechanism is source-neutral and
  applies to local or remote Kungfu runtimes.

## Residual risk

- The storage contract can drift if command semantics become source-specific.
  Shared schemas, fixtures, and multiple source-neutral smoke cases keep that
  boundary executable.
