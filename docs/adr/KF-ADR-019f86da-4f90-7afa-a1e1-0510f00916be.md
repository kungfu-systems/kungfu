...
- Subsystem: kfx contract — `framework/kfx` (the load plan and facet
  ...
  isolation) and the TUI reached through `kungfu tui` (Ink on libnode);
...
1. **Only one host actually loads a kfx, and the load rule is fused to it.**
   ...
   `View` component as the return value. The TUI (`kungfu tui`) loads no kfx
   at all.
...
The TUI clarifies what "two hosts" costs. `kungfu tui` is a Python CLI
