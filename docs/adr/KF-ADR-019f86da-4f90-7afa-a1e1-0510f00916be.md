...
adr_id: KF-ADR-019f86da-4f90-7afa-a1e1-0510f00916be
...
# KF-ADR-019f86da-4f90-7afa-a1e1-0510f00916be: dual-host kfx loading — a host-agnostic load plan and the background service facet on the OS-sandbox plane
...
- Subsystem: kfx contract — `framework/kfx` (the load plan and facet
  ...
  isolation) and the TUI reached through interactive bare `kungfu` (Ink on libnode);
  `framework/api/src/capability` (the capability host/guest protocol and the OS
  sandbox launcher).
- Related: KF-ADR-019f86da-4f90-7e5e-ae22-2a8fc24086f1 pinned the capability SDK contract and the runtime-tier
  declaration. KF-ADR-019f86da-4f90-79f1-8716-aca36b142847 put a trust boundary on the runtime plane — trust by
  verifiable origin, a default-deny OS sandbox, a trusted channel for zero-copy.
  KF-ADR-019f86da-4f90-7789-8b48-620aa694acf9 pinned the *uniform capability surface* so one facet source runs
...
1. **Only one host actually loads a kfx, and the load rule is fused to it.**
   ...
   `View` component as the return value. The TUI (interactive bare `kungfu`) loads no kfx
   at all.

2. **The runtime-plane sandbox is still unassembled** (KF-ADR-019f86da-4f90-7789-8b48-620aa694acf9). The OS-sandbox
...
The TUI clarifies what "two hosts" costs. Interactive bare `kungfu` is a Python CLI
...
itself and no host to launch it; and the OS-sandbox primitives keep their KF-ADR-019f86da-4f90-7789-8b48-620aa694acf9
...
- **Native-izing the macOS/Linux sandbox into libkungfu.** The OS sandbox stays
  ...
  because AppContainer has no CLI wrapper (see KF-ADR-019f86da-4f90-79f1-8716-aca36b142847/0014). Landing is
...
- **Read-scope narrowing** (Landlock / macOS profile read rules) — an KF-ADR-019f86da-4f90-7789-8b48-620aa694acf9
...
- **The service facet widens the attack surface** it must confine: a long-lived
  process is a larger target than a screen. Default-deny (KF-ADR-019f86da-4f90-79f1-8716-aca36b142847) and the
...
- **How a sandboxed service gets network/write — resolved: the user grants it,
  ...
  **Network granularity is coarse and platform-asymmetric — a known limitation.**
  ...
  is an KF-ADR-019f86da-4f90-7789-8b48-620aa694acf9 net-scope-narrowing follow-up. Informed consent must state
