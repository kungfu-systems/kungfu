...
PR #1624 adds immutable content publication by staged atomic rename, one fenced
writer per registry scope, generation and root preconditions, ordered lifecycle
history, retained prior roots, and durable applied or refused receipts. Refused
operations roll back staged payloads without disturbing prior content. Removal,
enablement, activation, and update are now Core-authoritative; reference-aware
garbage collection and the protected Agent Qualification Lab cutover remain
later stages, so implementation remains partial.
