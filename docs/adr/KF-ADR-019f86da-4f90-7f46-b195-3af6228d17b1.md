...
Initiative/Assignment product, but its domain vocabulary and operations currently live in
first-party Python/API/GUI code. The GUI also hard-codes a small list of
experience profiles.

Initiative/Assignment is one operator model, not a universal vocabulary. Other users may
organize reality as Week/Day/Action, Task/Job, Customer/Case/Decision, or a
model Kungfu cannot predict. Requiring those users to adopt Initiative/Assignment would
...
must not branch on Initiative, Assignment, Week, Day, Task, or another Profile noun.
...
Initiative/Assignment becomes the first first-party reference Profile Suite. It receives
no private admission, query, assessment, action, migration, or GUI authority.
Current authority uses one explicit Profile root and one native reader. A
cutover never installs a consumer-specific alias or alternate interpretation.

An independently authored Week/Day/Action Profile is the release oracle. Open
Profile support cannot be claimed until that Profile can coexist with Initiative
...
the canonical five questions through the public `initiative-home` member, and
...
- **Make Initiative/Assignment the universal workflow model.** Rejected because a
