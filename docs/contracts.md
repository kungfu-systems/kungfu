...
The root [`buildchain.kfd3.json`](../buildchain.kfd3.json) file is the product's
Buildchain-default KFD-3 registry and is the Buildchain-facing source of truth
...
that root registry through `./kungfu-code kfd:buildchain:check`.
...
Buildchain's default path and contract, declares `buildchain.kfd3.json` as its
