...
[`architecture.md`](../architecture/overview.md); for the principles behind them see
...
| Name | What it is |
|---|---|
| `kfx` | A **kungfu extension package** — the governed runtime artifact built on the extension contract. A kfx declares facets such as `view` or `adapter`; proposed `service` facets are documented in [`kfx-topology.md`](../architecture/kfx-topology.md). |
...
| `kungfu` | The **kungfu runtime binary and end-user CLI command** — the canonical way to invoke kungfu from the command line. Interactive bare `kungfu` opens the TUI; explicit arguments select CLI actions. It embeds a Python and a Node runtime and exposes the journal/state APIs; it is the runtime everything else runs on. Operator-facing slices such as managed runs and skill context injection grow under this command. |
...
| **schema owner** | The single authority for a persisted structured fact: either the Hana POD closed set or one `.fbs` schema, never both. Typed views and opaque content bodies are not additional schema owners. See [KF-ADR-019f86da-4f90-71eb-b4c0-376ca7bc7ad3](../adr/KF-ADR-019f86da-4f90-71eb-b4c0-376ca7bc7ad3.md). |
...
| **fact surface** | A declared family of domain facts inside a contract world. It has one schema owner plus explicit source, identity, time, correction, conflict, redaction, and known-limit semantics. See [KF-ADR-019f86da-4f90-7d81-90a0-d144fc27fe03](../adr/KF-ADR-019f86da-4f90-7d81-90a0-d144fc27fe03.md). |
...
| **action recorder** | The language-neutral C++ core surface for writing action facts into the journal. Python and Node expose thin bindings over it; they must not implement separate causality, writer, or receipt semantics. See [KF-ADR-019f86da-4f90-70f3-9a0e-d502826fbc81](../adr/KF-ADR-019f86da-4f90-70f3-9a0e-d502826fbc81.md). |
...
| **channel** | A source/destination communication **routing** edge between locations: it authorizes that one location's data may flow to another. Channels back runtime read/write/request paths; they are transport, not fact authority. |
| **outlet** | A named **output** stream a peer opens for itself — a dynamically created output location, writable from an off-main thread (formerly `band`). Distinct layer from a channel: a channel is routing (which locations may read which), an outlet is output creation. The off-main-thread writer pool that backs it (`off_thread_writers_`) is an orthogonal concern — single-writer isolation for external callback threads — not part of the outlet concept itself. See [KF-ADR-019f86da-4f90-72ff-a471-26c952d200e0](../adr/KF-ADR-019f86da-4f90-72ff-a471-26c952d200e0.md). |
...
| **Episode** | The first-class temporal substrate: a bounded causal segment across Fact Cuts. It is the storage/export/import/fsck/timeline-slicing unit; frame-level causality closes inside it, and cross-Episode influence is declared as Episode dependencies. See [KF-ADR-019f86da-4f90-791c-9b90-4888cca36327](../adr/KF-ADR-019f86da-4f90-791c-9b90-4888cca36327.md) and [`episode-object-model.md`](episode-object-model.md). |
...
See [`architecture.md`](../architecture/overview.md) for how these fit together.
