...
| **schema owner** | The single authority for a persisted structured fact: either the Hana POD closed set or one `.fbs` schema, never both. Typed views and opaque content bodies are not additional schema owners. See [ADR-0047](../adr/ADR-0047-authoritative-facts-hana-pod-or-flatbuffers.md). |
...
| **fact surface** | A declared family of domain facts inside a contract world. It has one schema owner plus explicit source, identity, time, correction, conflict, redaction, and known-limit semantics. See [ADR-0051](../adr/ADR-0051-kfd-contract-world-fact-admission-and-trust.md). |
...
| **event loop** | The **Python event-loop integration** (`LiveEventLoop`): fuses the engine loop with Python `asyncio` on a single thread — see [ADR-0003](../adr/ADR-0003-control-axis-python-coroutine-integration.md). |
