...
1. Is Nuitka painful enough to justify replacing? (If not, symmetry alone is
   not a reason — see `docs/development/rust-adoption.md`, "the team likes Rust" does not
   count.)
...
1. **Phase A** — a host shell binary behind an off-by-default flag, macOS
   ...
   cdylib/FFI case per `docs/development/rust-adoption.md` (mode 2 is deliberately
   non-default).
