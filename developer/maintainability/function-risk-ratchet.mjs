...
const PUBLIC_POLICY_PATH =
  'developer/maintainability/function-risk-policy.json';
const RATCHET_POLICY_PATH =
  'developer/maintainability/function-risk-ratchet-policy.json';

function sumRisk(functions) {
  return functions.reduce((sum, item) => sum + item.baseRisk, 0);
}

function renameMapFromBase(base) {
  const renamedFrom = new Map();
  const status = String(
    git(['diff', '--name-status', '--find-renames=50%', base, '--']),
  );
  for (const line of status.split('\n')) {
    const [kind, previous, current] = line.split('\t');
    if (/^R\d{3}$/u.test(kind || '') && previous && current)
      renamedFrom.set(current, previous);
  }
  return renamedFrom;
}

function projectRenamedBaseline(baseline, current, renamedFrom) {
  const renamedTo = new Map(
    [...renamedFrom].map(([currentPath, previousPath]) => [
      previousPath,
      currentPath,
    ]),
  );
  const currentOwners = new Map(
    current.functions.map((item) => [item.path, item.owner]),
  );
  const project = (item) => {
    const currentPath = renamedTo.get(item.path);
    return currentPath
      ? {
          ...item,
          path: currentPath,
          owner: currentOwners.get(currentPath) || item.owner,
        }
      : item;
  };
  return {
    ...baseline,
    files: baseline.files.map(project),
    functions: baseline.functions.map(project),
  };
}
...
function buildFunctionRiskRatchet(options = {}) {
  ...
  const renamedFrom = renameMapFromBase(base);
  ...
  const identity = {
    ...
    implementationRoot: digest(
      [
        'developer/maintainability/source-analysis-kernel.mjs',
        'developer/maintainability/source-analysis-session.mjs',
        'developer/maintainability/function-risk-transition.mjs',
  ...
  const cached = readThroughAnalysisCache(
    ...
    () => {
      ...
      const measuredBaseline = functionSnapshot(
        baselineInputs,
        publicPolicy,
        layers,
        ownership,
      ...
      const baseline = projectRenamedBaseline(
        measuredBaseline,
        current,
        renamedFrom,
      );
