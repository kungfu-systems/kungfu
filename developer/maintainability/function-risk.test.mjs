...
test('quality-governance changed functions inherit the protected ratchet', () => {
  const allowed = metric({
    id: 'javascript-typescript:developer/maintainability/a.mjs:allowed:1',
    path: 'developer/maintainability/a.mjs',
    symbol: 'allowed',
    owner: 'developer/maintainability',
    baseRisk: 87,
  });
  const blocked = metric({
    id: 'javascript-typescript:developer/maintainability/a.mjs:blocked:10',
    path: 'developer/maintainability/a.mjs',
    symbol: 'blocked',
    owner: 'developer/maintainability',
    baseRisk: 88,
  ...
  const current = {
    ...
    wrapperOnlyFindings: report.findings.filter(
      ({ code, paths }) =>
        ...
        paths.some((relative) =>
          relative.startsWith('developer/maintainability/'),
        ),
    ).length,
  };
...
test('phase-2 governance hotspots stay below the exact protected baseline without risk transfer', () => {
  ...
  const owner = 'developer/maintainability';
  ...
  const migrationGuards = current.filter(
    (item) =>
      item.path === 'developer/maintainability/function-risk-ratchet.mjs' &&
      ['projectRenamedBaseline', 'renameMapFromBase'].includes(item.symbol),
  );
  const existingGovernance = current.filter(
    (item) => !migrationGuards.includes(item),
  );
  assert.equal(migrationGuards.length, 2);
  assert.ok(
    migrationGuards.reduce((sum, item) => sum + item.baseRisk, 0) <= 46,
  );
  assert.ok(
    existingGovernance.reduce((sum, item) => sum + item.baseRisk, 0) < 2328,
  );
  ...
  const baselineHotspots = new Set(
    baseline
      .filter(
        (item) =>
          ['framework/maintainability', owner].includes(item.owner) &&
          item.baseRisk > 50,
      )
      .map(
        (item) =>
          `${item.path.replace(/^framework\/maintainability\//u, 'developer/maintainability/')}:${item.symbol}`,
      ),
  );
  assert.ok(
    current
