...
test('current semantic amplification projection has one route per family', () => {
  ...
  assert.equal(report.summary.mappedSurfaces, 101);
  assert.equal(
...
test('quality-governance topology fails closed on missing or parallel ownership', () => {
  ...
  const missingIssues = validateManifest(
    missing,
    layers,
    new Set(['developer/maintainability/source-analysis-kernel.mjs']),
  );
  assert.ok(
    missingIssues.some(
      ({ code, family: id }) =>
...
test('changed semantic identity outside the declared graph is blocking', () => {
  const broken = clone(manifest);
  broken.families[0].discoveryRoots = ['developer/maintainability/'];
  ...
  const issues = validateManifest(
    broken,
    layers,
    new Set(['developer/maintainability/semantic-amplification.mjs']),
  );
