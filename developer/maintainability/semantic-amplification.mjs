...
function serializeElectronBuilderProjection(name, config) {
  return [
    `# Generated by developer/maintainability/semantic-amplification.mjs from the canonical base and ${name} overlay.`,
    '# Run: node developer/maintainability/semantic-amplification.mjs --electron-builder-config-write',
...
function checkElectronBuilderProjections() {
  ...
  if (drift.length > 0) {
    throw new Error(
      `electron_builder_projection_drift:${drift.join(',')}; run node developer/maintainability/semantic-amplification.mjs --electron-builder-config-write`,
    );
  }
}
...
function buildReport(manifest, layers) {
  ...
  const body = {
    ...
    generatorRoot: digest(
      currentBytes('developer/maintainability/semantic-amplification.mjs'),
    ),
