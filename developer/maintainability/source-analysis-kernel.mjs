...
function isEligible(pathname, policy) {
  if (
    ...
    return false;
  const { paths: metadataPaths, prefixes: metadataPrefixes } =
    complexityMetadataRoutes(policy);
  if (
...
function complexityMetadataRoutes(policy) {
  const legacy = policy.legacyGovernancePaths || {};
  return {
    paths: [policy.baselinePath, ...(legacy.baselinePaths || [])],
    prefixes: [
      policy.waiverDirectory,
      policy.baselineGovernance?.transitionDirectory,
      ...(legacy.waiverDirectories || []),
      ...(legacy.transitionDirectories || []),
    ].filter(Boolean),
  };
}
