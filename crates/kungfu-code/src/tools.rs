...
// (KUNGFU_FNM_DIST_MIRROR / KUNGFU_UV_DIST_MIRROR), and versions / checksums
// can be overridden per environment (KUNGFU_FNM_VERSION / KUNGFU_UV_VERSION,
// KUNGFU_FNM_SHA256 / KUNGFU_UV_SHA256).
...
/// Pinned bootstrap versions. These only apply when a tool is absent and the
/// launcher has to fetch it; an fnm / uv already on PATH is used as-is.
const FNM_VERSION: &str = "1.39.0";
...
impl Tool {
    fn version(&self) -> String {
        env::var(self.version_env).unwrap_or_else(|_| self.default_version.to_string())
    }
...
pub fn find_tool(tool: &Tool) -> Option<PathBuf> {
    ...
    let cached = tool.cached_binary();
    cached.is_file().then_some(cached)
}

/// Resolve a tool, bootstrapping the pinned prebuilt release when absent.
pub fn ensure_tool(tool: &Tool) -> PathBuf {
    if let Some(found) = find_tool(tool) {
        return found;
    }
    bootstrap(tool).unwrap_or_else(|err| {
        util::die_code(
            &format!(
...
fn bootstrap(tool: &Tool) -> Result<PathBuf, String> {
    let url = tool.download_url().ok_or_else(|| {
        format!(
            "no prebuilt {} for {}/{}",
            tool.name,
            env::consts::OS,
