...
impl TemporaryContextRoot {
    fn create(command: &str) -> Result<Self, String> {
        use std::time::{SystemTime, UNIX_EPOCH};

        let nonce = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map_err(|error| format!("{command}: system clock error: {error}"))?
            .as_nanos();
        let path = std::env::temp_dir().join(format!(
            "kungfu-trunk-{command}-{}-{nonce}",
            std::process::id()
        ));
        std::fs::create_dir(&path)
            .map_err(|error| format!("{command}: cannot create temporary context: {error}"))?;
        Ok(Self { path })
    }

    fn as_str(&self, command: &str) -> Result<&str, String> {
        self.path
            .to_str()
            .ok_or_else(|| format!("{command}: temporary path is not valid UTF-8"))
    }
}
...
fn inspect_compact(args: PlanArgs) -> Result<(), String> {
    use kungfu_embedding::{Context, ContextConfig, StorageCompactPlanRequest, ABI_V1};

    let root = runtime_dir(&args)?;
    let context_root = TemporaryContextRoot::create("compact-plan")?;
    let context = Context::open(&ContextConfig::new(
        context_root.as_str("compact-plan")?,
        "kungfu-trunk",
        "compact-plan",
    ))
    .map_err(|error| error.to_string())?;
    ...
    let report = context
        ...
        .map_err(|error| error.to_string())?;
    emit("compact-plan", ABI_V1, &root, args.json, &report)
}
